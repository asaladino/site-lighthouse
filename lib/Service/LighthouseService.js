...
var _LighthouseDefault = _interopRequireDefault(require("../Config/LighthouseDefault"));

var _printer = _interopRequireDefault(require("lighthouse/lighthouse-cli/printer"));
...
class LighthouseService {
  constructor(args) {
    this.events = new Map();
    this.args = args;
    this.optionsRepository = new _OptionsRepository.default(this.args);
    this.option = this.optionsRepository.getOption();
    this.urlsRepository = new _UrlsRepository.default(this.option, this.args);
  ...
  start() {
    ...
    return _asyncToGenerator(function* () {
      ...
      }).then(chrome => _this.runReports(chrome)).catch(reason => console.log(reason));
    })();
  }
  ...
  runReports(chrome) {
    ...
    return _asyncToGenerator(function* () {
      ...
      let urls = _this2.urlsRepository.findAll().filter(url => {
        return !_fs.default.existsSync(_path.default.join(_this2.folder, url.name + '.json'));
      });

      let progress = new _Progress.default(null, urls.length);

      _this2.emitStart(progress);

      for (let url of urls) {
        let results = yield (0, _lighthouse.default)(url.url, flags, _LighthouseDefault.default);
        yield _printer.default.write(JSON.stringify(results), 'json', _path.default.join(_this2.folder, url.name + '.json'));
        progress.update(url);

        _this2.emitProgress(progress);
      }

      _this2.emitComplete(new _Progress.default(null, urls.length));
  ...
  createOutputFolder() {
    this.folder = _path.default.join(this.args.output.filename, this.args.getSiteName(), 'lighthouse');
